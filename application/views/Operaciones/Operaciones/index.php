                    ...
                    <div class="btn-toolbar justify-content-between mr-2" role="toolbar" aria-label="Toolbar with button groups">
                        <div class="btn-group mb-2" role="group" aria-label="First group">
                            <div class="page-title">
                                <h3 class="page-title textocambio">Operaciones</h3>              
                            </div>                            
                        </div>
                        <div class="input-group mb-2" >
                            <div class="toolbar" id="boton_list">
                                <button type="button" class="btn btn-primary mb-2 mr-2" title="Registrar" data-toggle="modal" data-target="#ModalRegistro" ModalRegistro="<?= site_url('Cochera/Modal_Operaciones') ?>" >
                ...
                <div class="widget-content widget-content-area br-6">
                    <div class="table-responsive mb-4 mt-4">
                        <table id="zero-config" class="table dt-table-hover" style="width:100%">
                            ...
                            <tbody>
                                <?php foreach($list_operaciones as $list){?> 
                                <tr>
                                    <td><?php echo $list['usuario_nombres']." ".$list['usuario_apater']." ".$list['usuario_amater'] ?></td>
                                    <td><?php echo $list['placa'] ?></td>
                                    <td><?php echo $list['descripcionopera'] ?></td>
                                    <td>
                                        <a href="javascript:void(0);"  title="Editar" data-toggle="modal" data-target="#ModalUpdate" ModalUpdate="<?= site_url('Cochera/Modal_Update_Operaciones') ?>/<?php echo $list["id_operaciones"]; ?>" >
                                        ...
                                        <a title="Eliminar" onclick="Delete_Operaciones('<?php echo $list['id_operaciones']; ?>')" id="delete" role="button">
...
<script>
    $(document).ready(function() {
        $("#lioperaciones").addClass('active');
        //$("#mconfiguracion").attr('aria-expanded','true');
        $("#operaciones_mod").addClass('active');
    });
</script>
